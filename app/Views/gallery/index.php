...
<section class="relative py-14 md:py-16 px-4 overflow-hidden" style="background: linear-gradient(135deg, #0F3D3E 0%, #1C1C1C 100%);">
    ...
    <div class="max-w-5xl mx-auto text-center relative z-10" data-aos="fade-up">
        ...
        <h1 class="text-4xl md:text-5xl font-light mb-4 leading-tight text-white" style="font-family: 'Cormorant Garamond', serif; letter-spacing: -0.02em;">
            Gallery of Elegant Celebrations
        </h1>
        <p class="text-base md:text-lg text-gray-300 max-w-3xl mx-auto leading-relaxed" style="font-family: 'Montserrat', sans-serif;">
            Discover curated highlights from weddings, proposals, birthdays, and corporate events crafted with precision and style.
        </p>
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-center mb-10">
            <p id="gallery-results-text" class="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold tracking-wider uppercase" style="background-color: rgba(15, 61, 62, 0.08); color: #0F3D3E; font-family: 'Montserrat', sans-serif;">
                Showing <?php echo count($items ?? []); ?> Events
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="gallery-grid">
            <?php foreach ($items as $index => $item): ?>
                <article 
                    class="gallery-card group relative bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2"
                    ...
                    <div class="relative aspect-[3/4] overflow-hidden">
                        ...
                        <!-- <div class="absolute inset-0 bg-gradient-to-br from-[#0F3D3E] to-[#C8A951]"></div> -->
                        ...
                        <div class="absolute top-4 left-4 z-10">
                            <span class="px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider backdrop-blur-md" style="background: rgba(200, 169, 81, 0.9); color: #0F3D3E; font-family: 'Montserrat', sans-serif;">
                                <?php echo htmlspecialchars($item['category_name'] ?? 'Event'); ?>
                            </span>
                        </div>
                        
                        <!-- Zoom Icon -->
                        <div class="absolute top-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                            <button type="button" class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white hover:bg-[#C8A951] hover:text-[#0F3D3E] transition-colors" onclick="openLightbox(<?php echo $index; ?>)">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"></path></svg>
                            </button>
                        </div>
                    </div>
                    
                    <!-- Card Content - Postcard Style -->
                    <div class="relative p-6 bg-white">
                        <!-- Decorative accent -->
                        <div class="absolute top-0 left-6 right-6 h-1 bg-gradient-to-r from-[#0F3D3E] via-[#C8A951] to-[#0F3D3E] opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                        
                        <div class="flex items-start justify-between gap-4 mb-3">
                            <h3 class="text-xl font-bold text-[#0F3D3E] group-hover:text-[#C8A951] transition-colors duration-300" style="font-family: 'Cormorant Garamond', serif;">
                                <?php echo htmlspecialchars($item['title']); ?>
                            </h3>
                            <span class="flex-shrink-0 w-8 h-8 rounded-full bg-[#F8F5F2] flex items-center justify-center text-[#C8A951] group-hover:bg-[#C8A951] group-hover:text-white transition-all duration-300">
                        ...
                        <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                            <div class="flex items-center gap-2 text-xs text-gray-500">
                                ...
                                <span><?php echo htmlspecialchars($item['category_name'] ?? 'Event'); ?></span>
                            </div>
                            <button 
                                type="button"
                                class="text-xs font-semibold text-[#0F3D3E] hover:text-[#C8A951] transition-colors flex items-center gap-1"
        ...
        <div id="filtered-empty-state" class="hidden text-center py-20">
            ...
            <h3 class="text-2xl font-bold text-[#0F3D3E] mb-2" style="font-family: 'Cormorant Garamond', serif;">No Matching Events</h3>
            <p class="text-gray-600 mb-6">Try another category or reset the filter.</p>
        ...
        <?php if (empty($items)): ?>
            <div class="text-center py-20">
                ...
                <h3 class="text-2xl font-bold text-[#0F3D3E] mb-2" style="font-family: 'Cormorant Garamond', serif;">No Events Found</h3>
                <p class="text-gray-600 mb-6">There are no events in this category yet. Check back soon!</p>
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <?php if (!empty($testimonials ?? [])): ?>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <?php foreach (($testimonials ?? []) as $index => $testimonial): ?>
                    ...
                    ?>
                    <article class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow relative" data-aos="fade-up" data-aos-delay="<?php echo $index * 100; ?>">
                        ...
                        <p class="text-gray-700 mb-6 leading-relaxed italic">
                            "<?php echo htmlspecialchars($testimonial['content'] ?? 'Sapphire Events made our celebration seamless, elegant, and unforgettable.'); ?>"
                        </p>
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="font-bold text-[#0F3D3E]"><?php echo htmlspecialchars($name); ?></p>
                                <p class="text-sm text-gray-500">Verified Client</p>
                            </div>
                        </div>
                    </article>
                <?php endforeach; ?>
        ...
        <?php else: ?>
            <div class="text-center py-8">
                <p class="text-gray-600">Client testimonials will appear here soon.</p>
            </div>
        <?php endif; ?>
    </div>
</section>
...
<div id="lightbox-modal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="relative w-full h-full flex items-center justify-center p-4 md:p-8">
        <!-- Close Button -->
        <button onclick="closeLightbox()" class="absolute top-4 right-4 md:top-6 md:right-6 z-50 w-12 h-12 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-[#C8A951] hover:text-[#0F3D3E] transition-all">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>
        
        <!-- Navigation -->
        <button onclick="navigateLightbox(-1)" class="absolute left-2 md:left-6 top-1/2 -translate-y-1/2 z-50 w-12 h-12 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-[#C8A951] hover:text-[#0F3D3E] transition-all">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
        </button>
        
        <button onclick="navigateLightbox(1)" class="absolute right-2 md:right-6 top-1/2 -translate-y-1/2 z-50 w-12 h-12 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-[#C8A951] hover:text-[#0F3D3E] transition-all">
...
function filterGallery(category) {
    ...
    if (resultsText) {
        resultsText.textContent = `Showing ${visibleCount} Event${visibleCount === 1 ? '' : 's'}`;
    }
...
function openLightbox(index) {
    ...
    category.textContent = item.category_name || 'Event';
